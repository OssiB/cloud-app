...
public class TestTopTitleMapper {
	...
	public void processRecord() throws IOException{
		...
		List<Pair<NullWritable, TopTitles.TextArrayWritable>> result=mapDriver.run();
		assertEquals(new Text("(2, student)"),(Text)result.get(0).getSecond().get()[0]);
		assertEquals(new Text("(3, answer)"),(Text)result.get(0).getSecond().get()[1]);
